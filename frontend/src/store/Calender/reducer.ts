...
import {ActionTypes, CALENDER_ACTIONS, ActivityList} from "./types";
...
export const calender: Reducer<State, ActionTypes> = (state = initialState, action) => {
  switch (action.type) {
    ...
    case CALENDER_ACTIONS.DELETE_ACTIVITIES:
      return {...state, activityList: days.map((d) => state.activityList[d.id].filter((a) => !action.response[d.id].includes(a)))}
